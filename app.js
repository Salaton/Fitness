...
myDoc.fontSize(20);
myDoc.text("Welcome to the gym!", 100,100);
myDoc.end();
...
app.get("/", function(req,res){ 
    Trainer.find({}, function(err, trainer){
       if(err){
          ...
          res.render("index",{trainer:trainer});
       }
    });
});
...
app.post("/enroll",isLoggedIn, function(req,res){
    Enroll.create(new Enroll({username:req.body.username, email:req.body.email, fullname:req.body.fullname, enroll:req.body.enroll }),function(err, enroll){
        if(err){
            console.log(err);
...
app.post("/Dashboard/trainer", function(req,res){
    Trainer.create(new Trainer({email: req.body.email, firstname:req.body.firstname, lastname:req.body.lastname,  address: req.body.address, service: req.body.service, city:req.body.city,  description: req.body.description, image: req.body.image}),function(err, trainer){
        if(err){
            console.log(err);
...
app.post("/Dashboard/user", function(req,res){
    Admin.create(new Admin({username: req.body.username, email: req.body.email, firstname:req.body.firstname, lastname:req.body.lastname,  address: req.body.address, city:req.body.city}),function(err, admin){
        //delete Admin.city;
        if(err){
            console.log(err);
...
app.post("/Dashboard/classes", function(req,res){
    Class.create(new Class({class: req.body.class, image:req.body.image, description:req.body.description}),function(err, classes){
        if(err){
            console.log(err);
...
app.post("/signup", function(req, res){
    ...
    User.register(new User({username: req.body.username, fullname:req.body.fullname, email: req.body.email, phoneno: req.body.phoneno, address: req.body.address}),req.body.password,  function(err, user){
        if(err){
            console.log(err);
...
app.post('/adminlogin', requireAdmin(), passport.authenticate('local',{
